...
在 S6 完成后进入 S7 前，必须执行以下步骤：

1. 执行根目录脚本：`./acceptance-up.sh`
2. 必须回传验收地址与日志路径
3. 必须通过健康检查：
...
当 `S7 用户验收通过` 后，必须执行分支清理：

1. 删除验收分支（`acceptance/*` 或约定的验收分支名）
2. 同时清理本地与远端验收分支
3. 若该分支仍承载未完成任务，需先在 Issue 说明并获得用户确认后再保留
...
在 Issue body 中新增以下字段：

```markdown
## 验收信息
- 验收地址: 
- 启动命令: `./acceptance-up.sh`
- 停止命令: `./acceptance-down.sh`
- 日志路径: `.acceptance-logs/backend.log`, `.acceptance-logs/frontend.log`
```
